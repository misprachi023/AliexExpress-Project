...
const LifeStyle=()=>{
  const slidesPerView=useBreakpointValue({base:2,md:4,lg:5})
    return (
        <>
         <Box padding="20px" fontSize="25px"> Shop by <span style={{fontWeight:"bold"}}>Lifestyle</span></Box>
          <Swiper
          ...
          >
            {LifeStyleSlide.map((item) => (
              <SwiperSlide>
                <Box fontSize={["sm","md","md"]} margin="10px" >
                  <img src={item.image} style={{ width: "100%" ,height: "auto", padding: "5px" }} alt={item.title} />
                  <Text textAlign="center" fontWeight="bold" >{item.title} </Text>
                 
                  <Button color="blue" backgroundColor="white" >View All →</Button>
                </Box>
              </SwiperSlide>
            ))}
          </Swiper>
